"""
GCIVL State-based Dual Agent with Cross-Attention Enhancement.

This agent combines:
1. Dual Goal Representation: φ∨(g) = [d*(s_i, g)]_{i=1}^K
2. Cross-Attention: φ(g|s) = φ∨(g) + CrossAttn(φ∨(g), ψ(s))
...
class GCIVLDualCrossAttnAgent(flax.struct.PyTreeNode):
    """GCIVL agent with Dual goal representation enhanced by Cross-Attention (state-based)."""
    rng: Any
    network: Any
    ...
    def value_loss(self, batch, grad_params):
        ...
        # Get base dual goal representation
        goal_reps = self.network.select('rep_value')(batch['value_goals'])
        
        # Apply Cross-Attention to make goal representation state-aware
    ...
    def actor_loss(self, batch, grad_params, rng=None):
        ...
        # Get state-aware goal representation for actor
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='gcivl_dual_crossattn',
            lr=3e-4,
            batch_size=1024,
