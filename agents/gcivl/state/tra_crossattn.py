"""
GCIVL State-based TRA Agent with Cross-Attention Enhancement.
...
class GCIVLTRACrossAttnAgent(flax.struct.PyTreeNode):
    """GCIVL agent with TRA goal representation enhanced by Cross-Attention (state-based)."""
    rng: Any
    network: Any
    ...
    def value_loss(self, batch, grad_params):
        ...
        _, _, goal_reps = self.network.select('contrastive')(
            batch['observations'], batch['value_goals'], actions=None, info=True
        )
        ...
        _, _, grad_goal_reps = self.network.select('contrastive')(
            batch['observations'], batch['value_goals'], actions=None, info=True
        )
    ...
    def sample_actions(
        ...
        _, _, goal_reps = self.network.select('contrastive')(
            observations, goals, actions=None, info=True
        )
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='gcivl_tra_crossattn',
            lr=3e-4,
            batch_size=1024,
