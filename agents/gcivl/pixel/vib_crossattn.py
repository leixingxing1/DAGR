"""
GCIVL Visual VIB Agent with Cross-Attention Enhancement.
...
class GCIVLVisualVIBCrossAttnAgent(flax.struct.PyTreeNode):
    """GCIVL Visual agent with VIB goal representation enhanced by Cross-Attention."""
    rng: Any
    network: Any
    ...
    def value_loss(self, batch, grad_params, rng):
        ...
        goal_reps, kl_loss, kl_info = self.network.select('vib')(goals, rng, encoded=True, params=grad_params)
        
        goal_reps_stopped = jax.lax.stop_gradient(goal_reps)
        goal_reps_obs = self.network.select('cross_attention')(goal_reps_stopped, obs)
        goal_reps_next = self.network.select('cross_attention')(goal_reps_stopped, next_obs)
        ...
        goal_reps_aware = self.network.select('cross_attention')(goal_reps, grad_obs, params=grad_params)
        (v1, v2) = self.network.select('value')(grad_obs, goal_reps_aware, params=grad_params)
        v = (v1 + v2) / 2
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        goal_reps, _, _ = self.network.select('vib')(goals, rng, encoded=True)
        ...
        v1, v2 = self.network.select('value')(obs, jax.lax.stop_gradient(goal_reps_obs))
        nv1, nv2 = self.network.select('value')(next_obs, jax.lax.stop_gradient(goal_reps_next))
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
        adv = nv - v
        ...
        grad_goal_reps, _, _ = self.network.select('vib')(grad_goals, rng, encoded=True, params=grad_params)
        grad_goal_reps_aware = self.network.select('cross_attention')(grad_goal_reps, grad_obs, params=grad_params)
    ...
    def sample_actions(self, observations, goals=None, seed=None, temperature=1.0):
        ...
        goal_reps, _, _ = self.network.select('vib')(goals, vib_seed, encoded=True)
    ...
    def create(cls, seed, ex_observations, ex_actions, config, ex_goals=None):
        ...
        vib_def = VIB(
            encoder=MLP(hidden_dims=config['vib_hidden_dims'], layer_norm=config['layer_norm']),
            beta=config['beta'], rep_dim=config['goalrep_dim'],
        )
        ...
        network_info = dict(
            ...
            vib=(vib_def, (ex_encoder_outputs, jax.random.PRNGKey(0), True)),
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='gcivl_vib_cross_vis',
            ...
            const_std=True, discrete=False, goalrep_dim=256, beta=0.001,
