"""
GCIVL Visual BYOL Agent with Cross-Attention Enhancement.
...
class GCIVLVisualBYOLCrossAttnAgent(flax.struct.PyTreeNode):
    """GCIVL Visual agent with BYOL goal representation enhanced by Cross-Attention."""
    rng: Any
    network: Any
    ...
    def byol_loss(self, batch, grad_params):
        ...
        return forward_loss + backward_loss, {'forward_loss': forward_loss, 'backward_loss': backward_loss}
    ...
    def value_loss(self, batch, grad_params):
        ...
        goal_rep_obs = self.network.select('cross_attention')(goal_rep, obs)
        goal_rep_next = self.network.select('cross_attention')(goal_rep, next_obs)

        (next_v1_t, next_v2_t) = self.network.select('target_value')(next_obs, goal_rep_next)
        next_v_t = jnp.minimum(next_v1_t, next_v2_t)
        q = batch['rewards'] + self.config['discount'] * batch['masks'] * next_v_t

        (v1_t, v2_t) = self.network.select('target_value')(obs, goal_rep_obs)
        v_t = (v1_t + v2_t) / 2
        adv = q - v_t
        ...
        grad_goal_rep_aware = self.network.select('cross_attention')(
            grad_goal_rep, grad_obs, params=grad_params
        )
        
        (v1, v2) = self.network.select('value')(grad_obs, grad_goal_rep_aware, params=grad_params)
        v = (v1 + v2) / 2
    ...
    def actor_loss(self, batch, grad_params, rng=None):
        ...
        v1, v2 = self.network.select('value')(obs, goal_rep_obs)
        nv1, nv2 = self.network.select('value')(next_obs, goal_rep_next)
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
        adv = nv - v
        ...
        dist = self.network.select('actor')(grad_obs, grad_goal_rep_aware, params=grad_params)
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='gcivl_byol_cross_vis',
