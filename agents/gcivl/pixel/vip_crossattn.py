"""
GCIVL Visual VIP Agent with Cross-Attention Enhancement.
...
class GCIVLVisualVIPCrossAttnAgent(flax.struct.PyTreeNode):
    """GCIVL Visual agent with VIP goal representation enhanced by Cross-Attention."""
    rng: Any
    network: Any
    ...
    def rep_loss(self, batch, grad_params):
        ...
        grad_final_obs = self.network.select('goal_encoder')(batch['rep_final_obs'], params=grad_params)
        grad_init_obs = self.network.select('state_encoder')(batch['rep_init_obs'], params=grad_params)
        grad_k_obs = self.network.select('state_encoder')(batch['rep_k_obs'], params=grad_params)
        grad_next_k_obs = self.network.select('state_encoder')(batch['rep_k+1_obs'], params=grad_params)

        phi_g = self.network.select('rep')(grad_final_obs, params=grad_params)

        o_0 = self.network.select('rep')(grad_init_obs, params=grad_params)
        o_k1 = self.network.select('rep')(grad_k_obs, params=grad_params)
        o_k2 = self.network.select('rep')(grad_next_k_obs, params=grad_params)
    ...
    def value_loss(self, batch, grad_params):
        ...
        grad_goal_reps = self.network.select('rep')(grad_goals)
    ...
    def create(cls, seed, ex_observations, ex_actions, config, ex_goals=None):
        ...
        network_info = dict(
            ...
            rep=(rep_def, (ex_encoder_outputs,)),
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='gcivl_vip_cross_vis',
            ...
            dataset_class='VIPDataset', oraclerep=False, norm=False,
