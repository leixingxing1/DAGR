...
class GCValue(nn.Module):
    ...
    Attributes:
        hidden_dims: Hidden layer dimensions.
        layer_norm: Whether to apply layer normalization.
        ensemble: Whether to ensemble the value function (bool), or number of ensemblized value functions (int).
        gc_encoder: Optional GCEncoder module to encode the inputs.
    """

    hidden_dims: Sequence[int]
    layer_norm: bool = True
    ensemble: Union[bool, int] = True
    gc_encoder: nn.Module = None

    def setup(self):
        mlp_module = MLP
        # Handles both n-ensembles and boolean for 2-ensemble.
        if type(self.ensemble) is int and self.ensemble > 1:
            mlp_module = ensemblize(mlp_module, self.ensemble)
        elif self.ensemble is True:
...
class GCMRNValue(nn.Module):
    ...
    def __call__(self, observations, goals, is_phi=False, info=False):
        ...
        if info:
            if self.ensemble:
                return v, phi_s.mean(axis=0), phi_g.mean(axis=0)
            else:
...
# ==============================================================================
# MINE Network for Mutual Information Estimation (Information Bottleneck)
# ==============================================================================

class MINENetwork(nn.Module):
    """
    MINE (Mutual Information Neural Estimation) statistics network.
    
    Computes T(x, z) for the Donsker-Varadhan representation:
        I(X;Z) >= E_P[T(x,z)] - log(E_Q[exp(T(x,z_marg))])
    
    This provides a lower bound on mutual information that can be optimized
    via gradient descent. Based on BC-IB paper implementation.
    
    Used in Dual-IB agent for estimating I(G; φ(G)) to apply information
    bottleneck regularization on goal representations.
    ...
    def __call__(self, x, z):
        """
        Compute T(x, z) statistics for MINE.
        
        Args:
            x: First input (e.g., original goal observation or encoded goal features)
               Shape: (batch_size, x_dim)
            z: Second input (e.g., compressed goal representation φ(g))
               Shape: (batch_size, z_dim)
            
        Returns:
            Scalar statistics for each sample, shape: (batch_size,)
        """
        # Flatten inputs to handle various input shapes
        x_flat = x.reshape(x.shape[0], -1)
        z_flat = z.reshape(z.shape[0], -1)
        
        # Concatenate the two inputs
        xz = jnp.concatenate([x_flat, z_flat], axis=-1)
        
        # MLP to compute T(x, z)
        h = xz
